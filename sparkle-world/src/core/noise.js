// Seeded simplex noise (2D + 3D) with fractal helpers, used by world generation.
// Based on the public-domain reference by Stefan Gustavson, with a seeded permutation.
...
export class Noise {
  ...
  /** Fractal 2D noise (sum of octaves), normalized to about -1..1. */
  fbm2(x, y, octaves = 4, lacunarity = 2, gain = 0.5) {
    let sum = 0, amp = 1, freq = 1, norm = 0;
    for (let o = 0; o < octaves; o++) {
      sum += this.n2(x * freq + o * 17.3, y * freq - o * 9.1) * amp;
      norm += amp;
      amp *= gain;
      freq *= lacunarity;
    }
  ...
  /** Fractal 3D noise, normalized to about -1..1. */
  fbm3(x, y, z, octaves = 3, lacunarity = 2, gain = 0.5) {
    let sum = 0, amp = 1, freq = 1, norm = 0;
    for (let o = 0; o < octaves; o++) {
      sum += this.n3(x * freq, y * freq, z * freq + o * 31.7) * amp;
      norm += amp;
      amp *= gain;
      freq *= lacunarity;
    }
  ...
  /** Ridged 2D noise in 0..1 (sharp crests), handy for paths and rivers. */
  ridge2(x, y, octaves = 3) {
    return 1 - Math.abs(this.fbm2(x, y, octaves));
  }
}
