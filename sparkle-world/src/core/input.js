...
export class Input {
  ...
  press(name, isDown) {
    if (name in this.virtual) this.virtual[name] = !!isDown;
  }
  ...
  update() {
    ...
    this.move.z = on ? z : 0;
    this.turn = on ? turn : 0;
    this.jump = on && (k.has('Space') || this.virtual.jump);
    this.down = on && (shift || k.has('KeyC') || this.virtual.down);
    this.run = on && (shift || joyRun || this.virtual.run);
    if (!on) {
      this.look.dx = 0;
      this.look.dy = 0;
    }
  ...
  _up(e, cancelled) {
    ...
    if (p.role === 'joystick') {
      ...
      this.joy.x = this.joy.z = 0;
      this._placeJoystick(null, null);
      // a quick touch on the joystick side that barely moved still counts as a tap
      const quick = performance.now() - p.t0 < 300 && Math.hypot(p.x - p.sx, p.y - p.sy) < 12;
      if (!cancelled && quick) {
        this.pointer = this.toNDC(p.x, p.y);
    ...
    if (p.hold) {
      this.events.emit('hold', { phase: 'end', ...this.toNDC(p.x, p.y), pointerType: p.type });
      return;
    }
