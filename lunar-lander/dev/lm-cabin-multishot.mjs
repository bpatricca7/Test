// Multi-view screenshots of dev/lm-cabin.html in ONE browser session (LM-CABIN helper).
// usage: node dev/lm-cabin-multishot.mjs '<query>' '<views JSON>' [prefix] [waitMs]
//   views: [{ "name": "fwd", "yaw": 0, "pitch": -10, "fov": 75, "station": "CDR", "eval": "js before",
//            "after": "js after the view is set" }, ...]
//   screenshots go to shots/lm-cabin/<prefix>-<name>.png
...
await server.listen();
const url = `http://127.0.0.1:${server.httpServer.address().port}/dev/lm-cabin.html?${query}`;
...
for (const v of views) {
  ...
  // wait for a few frames to be rendered with the new view (SwiftShader: ~1 frame/s)
  const f0 = await page.evaluate(() => window.H.frames);
  const need = v.frames ?? 3;
  for (let i = 0; i < 600; i++) {
    if ((await page.evaluate(() => window.H.frames)) >= f0 + need) break;
