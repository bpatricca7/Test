...
export function buildBody(id, rig, mats, rnd) {
  ...
  for (const [k, s] of [['L', 1], ['R', -1]]) {
    ...
    let prof;
    if (id === 'maya') {
      prof = [[-0.07, 0.026, 0.03, 0], [-0.035, 0.04, 0.046, 0], [0.0, 0.049, 0.052, 0], [0.04, 0.054, 0.055, 0], [0.09, 0.05, 0.052, 0], [0.16, 0.047, 0.048, 0],
      ...
      prof = [[-0.075, 0.028, 0.032, 0], [-0.038, 0.046, 0.052, 0], [0.0, 0.055, 0.058, 0], [0.05, 0.06, 0.062, 0], [0.11, 0.057, 0.058, 0], [0.18, 0.054, 0.055, 0],
    ...
    const ast = prof.map(([a, rx, rz, m]) => ({
      c: a < 0 ? sh.clone().addScaledVector(inward, -a * 0.9).add(V3(0, a * 0.45, 0)) : sh.clone().addScaledVector(dir, a),
